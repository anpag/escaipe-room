...
const Inventory = ({ items }) => (
  <div className="w-full bg-slate-900/80 border border-slate-700 rounded-lg p-3 shadow-lg flex flex-col mb-4">
    ...
    <div className="flex flex-row gap-3 overflow-x-auto pb-2">
      {items.length === 0 ? <div className="text-slate-600 text-sm italic py-2">Empty...</div> : items.map((item, index) => (
        <div key={index} className="flex items-center gap-2 bg-slate-800 border border-slate-600 rounded p-2 min-w-[140px] shrink-0" title={item.name}>
...
function App() {
  ...
  useEffect(() => { coordinatorEndRef.current?.scrollIntoView({ behavior: "smooth" }); }, [coordinatorMessages, isCoordinatorOpen]);
  ...
  // --- WebSocket Logic: Coordinator ---
  useEffect(() => {
    if (isCoordinatorOpen && activeTeam) {
  ...
  const handleZoneClick = (item) => {
    ...
    setMessages([]); // Clear previous messages, wait for WS to load history/initial
  };
  ...
  const handleReset = async () => {
    ...
    setInventory([]); setMessages([]); setCurrentRoom("databricks-room"); setIsRoomCompleted(false);
    ...
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-8 font-mono relative">
      <div className="flex-1 flex flex-col items-center justify-center h-full w-full max-w-6xl">
          <header className="w-full mb-4 flex justify-between items-center z-10">
            <div className={`flex items-center gap-2 ${currentTheme.color}`}><CurrentIcon size={24} /><h1 className="text-xl font-bold tracking-wider">{currentTheme.name.toUpperCase()}</h1></div>
            <div className="flex gap-2">
              {isRoomCompleted && <button onClick={handleNextRoom} className="px-4 py-2 rounded text-sm font-bold bg-blue-600 text-white animate-pulse">NEXT ROOM &gt;&gt;</button>}
              <button onClick={() => setIsCoordinatorOpen(!isCoordinatorOpen)} className={`px-4 py-2 rounded text-sm font-bold transition-colors ${isCoordinatorOpen ? 'bg-emerald-600 text-white' : 'bg-slate-700 text-emerald-400'}`}>MISSION CONTROL</button>
              <button onClick={handleReset} className="px-4 py-2 rounded text-sm font-bold bg-red-600/80 text-white">RESET</button>
              <button onClick={() => setDebugMode(!debugMode)} className={`px-4 py-2 rounded text-sm font-bold transition-colors ${debugMode ? 'bg-yellow-500 text-black' : 'bg-slate-700 text-slate-300'}`}>{debugMode ? 'DEBUG: ON' : 'DEBUG: OFF'}</button>
            </div>
          </header>
          ...
          <div className="relative w-full border-2 border-slate-700 rounded-lg overflow-hidden shadow-2xl bg-black">
            <div ref={containerRef} className={`relative w-full h-full ${debugMode ? 'cursor-crosshair' : ''}`} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp}>
                {currentRoom === "databricks-room" ? (
                  <video key={isRoomCompleted ? "end" : "bg"} src={`/assets/databricks-room-${isRoomCompleted ? "end" : "background"}.mp4`} autoPlay loop muted playsInline className="w-full h-auto object-contain pointer-events-none" style={{ filter: currentTheme.filter }} />
                ) : (
                  <img src={`/assets/${currentRoom}.png`} alt="Room" className="w-full h-auto object-contain pointer-events-none" style={{ filter: currentTheme.filter }} onError={(e) => {e.target.src="/assets/databricks-room.png"}} />
                )}
                ...
                {!debugMode && (roomConfig.zones || []).map((zone) => (
                  <div key={zone.id} onClick={() => handleZoneClick(zone)} className="absolute cursor-pointer hover:bg-white/10 transition-all duration-300" style={zone.style} />
                ))}
            </div>
          </div>
        </div>
      ...
      {isCoordinatorOpen && (
        <div className="fixed right-8 bottom-8 w-80 h-[500px] bg-slate-900/95 border border-emerald-500/50 rounded-lg shadow-2xl flex flex-col overflow-hidden backdrop-blur-md z-40">
            <div className="bg-emerald-900/30 p-3 flex justify-between items-center text-emerald-400 font-bold"><div className="flex items-center gap-2"><Activity size={16} /><span>MISSION CONTROL</span></div><button onClick={() => setIsCoordinatorOpen(false)}><X size={18} /></button></div>
            <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-emerald-900/50">
                {coordinatorMessages.map((msg, idx) => (
                    <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[90%] rounded p-2 text-xs ${msg.role === 'user' ? 'bg-emerald-600/20 text-emerald-100 border border-emerald-500/30' : 'text-emerald-300'}`}>{msg.text}</div>
                    </div>
                ))}
                <div ref={coordinatorEndRef} />
            </div>
            <form onSubmit={handleCoordinatorSend} className="p-3 bg-black/20 border-t border-emerald-500/20 flex relative">
                <input type="text" value={coordinatorInput} onChange={(e) => setCoordinatorInput(e.target.value)} placeholder="Request support..." className="w-full bg-slate-950/50 text-emerald-100 border border-emerald-500/30 rounded py-2 pl-3 pr-10 text-xs focus:outline-none focus:border-emerald-500" />
                <button type="submit" className="absolute right-4 top-5 text-emerald-500"><Send size={14} /></button>
            </form>
        </div>
      )}
